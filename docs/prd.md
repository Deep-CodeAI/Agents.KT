...
- [~] KSP annotation processor for compile-time `@Generable` (replaces runtime reflection); constrained decoding (Ollama/vLLM) + guided JSON mode (Anthropic/OpenAI). **Validation pass shipped (#1700)** — compile-time errors for non-sealed interfaces, annotation classes, enums, abstract classes, classes without a parameterised primary constructor. **Schema-generation pass shipped (#1701)** — `*__GeneratedSchema.kt` per non-sealed `@Generable data class`, byte-identical to the runtime path. **Sealed-root schema gen shipped (#1702)** — same generated-file mechanism extended to `@Generable sealed` types, emitting the `{"oneOf":[...]}` shape with `"type"` discriminators per variant. **`toLlmDescription` codegen shipped (#1703)** — second `LLM_DESCRIPTION` const in the same generated object, byte-identical to the runtime markdown; multi-constant cache (`GeneratedMetaCache`) loads all string fields in one pass and exposes typed `lookupJsonSchema` / `lookupLlmDescription` methods. `@LlmDescription` overrides are baked into the constant so runtime stays reflection-free for that path too. Still ahead: `constructFromMap` codegen (typed deserialization, the last reflection user); cross-module sealed detection; then drop `kotlin-reflect` from runtime classpath (Phase 3 of docs/ksp-design.md).
