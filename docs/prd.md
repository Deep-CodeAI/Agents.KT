...
### 12.6 AGNTCY Interoperability *(core shipped — OASF export/import + Identity-verify + DIR StoreService client; DIR Routing/Search + OCI referrers are follow-ups)*
...
**Server-side (expose agents.kt as an NLWeb endpoint) — SHIPPED (#4542).** `NlWebServer.from(agent).start()` serves the NLWeb `POST /ask` contract over a loopback JDK `HttpServer` — the **same `from(agent)` shape, serve surface, and threat model as `McpServer` / `A2AServer`** (loopback-only, optional bearer auth, front with a gateway), *not* a from-scratch RAG service. The query is the agent's input; an `NlWebSearchResult` output is served verbatim (ranked schema.org `results[]`), any other output becomes the `summary` answer. The agent does the retrieval/ranking, which you back with the **RAG `EmbeddingStore` seam** (`:agents-kt-rag`) or anything. (The benchmark caveat still holds — an independent study (Univ. Mannheim, [arXiv 2511.23281](https://arxiv.org/abs/2511.23281)) finds NLWeb *ties* plain RAG on effectiveness — so the value here is **standardization/interop** (serving the agentic web the same way we serve MCP + A2A), not raw retrieval performance.) The `/mcp` face — every NLWeb endpoint is also an MCP server — is `McpServer`'s domain (expose an `ask` skill there); `NlWebServer` is the `/ask`-over-HTTP path. So agents.kt now both **consumes** (the `nlwebSearch` tool, #4541) and **serves** (`NlWebServer`, #4542) NLWeb, completing the agent↔web-content layer.
