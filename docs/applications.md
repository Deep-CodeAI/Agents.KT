...
| **Code-review bots** — typed structured outputs that flow into existing JVM tooling. | `@Generable` outputs deserialise into your domain types directly — no JSON-to-class glue layer; constrained decoding wired across all eight providers. |
...
| **Resilient agent pipelines** — long-running flows where transient model failures shouldn't tear the whole job down. | `onLLMError { e -> Retry(maxAttempts) | RespondWith(fallback) | Rethrow }` — typed recovery from down providers / 5xx / malformed responses: exponential-backoff retries for transient failures, fallback values for hard ones; original exception identity preserved for `onError` observers. |
