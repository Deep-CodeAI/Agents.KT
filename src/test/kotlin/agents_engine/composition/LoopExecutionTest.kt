...
class LoopExecutionTest {
    ...
    fun loopRunsOnceWhenBlockImmediatelyReturnsNull() {
        ...
        val loop = agent<Int, Int>("inc") {
            execute { input -> count.add(input); input + 1 }
        }.loop { null }
    ...
    fun loopFeedsOutputBackAsInput() {
        ...
        val loop = agent<Int, Int>("inc") {
            execute { input -> log.add(input); input + 1 }
    ...
    fun loopOnPipelineWorks() {
        val pipeline = agent<Int, Int>("add1") { execute { it + 1 } } then
                       agent<Int, Int>("add1") { execute { it + 1 } }
    ...
    fun loopIsComposableInPipeline() {
        val prepare  = agent<String, Int>("len")     { execute { it.length } }
        val process  = agent<Int, Int>("inc")        { execute { it + 1 } }
        val finalize = agent<Int, String>("wrap")    { execute { "result:$it" } }
