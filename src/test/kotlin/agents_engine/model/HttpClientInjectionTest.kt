...
/**
 * #2385 — provider clients accept an optional `httpClient: HttpClient?`
 * so a long-lived process can share one connection pool / executor /
 * proxy across every agent in the JVM. When the parameter is null,
 * each client builds its own (byte-for-byte unchanged behavior).
 *
 * The internal `httpClient` accessor is the stable test seam — same
 * shape as the existing `internal fun sendChat` / `buildRequestJson`
 * seams on these clients.
 */
class HttpClientInjectionTest {
    ...
    fun `OpenAiClient honors injected HttpClient`() {
        val client = OpenAiClient(apiKey = "k", model = "test", httpClient = shared)
        assertSame(shared, client.httpClient)
    }

    @Test
    fun `ClaudeClient honors injected HttpClient`() {
        val client = ClaudeClient(apiKey = "k", model = "test", httpClient = shared)
        assertSame(shared, client.httpClient)
    }

    @Test
    fun `DeepSeekClient inherits injected HttpClient through OpenAi superclass`() {
        val client = DeepSeekClient(apiKey = "k", model = "test", httpClient = shared)
        assertSame(shared, client.httpClient)
    }
    ...
    fun `ModelConfig forwards httpClient through defaultClientFor for every provider`() {
        // #2385: the ModelConfig DSL slot must reach the per-provider client.
        // Without this, the injection API exists on the bare constructors but
        // isn't reachable from the `model { }` block — defeating the use case.
        listOf(
            ...
            val resolved = when (client) {
                ...
                is OpenAiClient -> client.httpClient
