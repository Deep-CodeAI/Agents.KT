...
### Added — HITL epic (#2487 in progress)

- **HITL interrupt/resume primitive (#2488)** — a tool inside an agentic loop can call `interrupt(payload)` to pause. The runtime captures a `SessionSnapshot` at the pre-tool-result boundary, fires `onTurnCheckpoint`, and throws `AgentInterruptException(snapshot, payload, pendingToolCallId)`. The caller persists the snapshot (any `SnapshotStore`, including filename-safe `FileSnapshotStore` per #2753), asks a human, then resumes via `agent.invokeSuspendResuming(input, resumeFrom = snapshot, resumeWith = reply)`. The runtime synthesises a tool result message from `resumeWith` via `toLlmInput` (so `@Generable` replies become JSON), and the loop continues — the model sees a continuous conversation. Composes with the manifest-hash restore guard (#2754, refuses to resume across an agent rebuild unless `allowManifestMismatch = true`), per-agent memory restore (#2755, shared-bank topologies safe), and `Pipeline`/`Branch`/`Loop`/`Parallel` composition operators (the exception propagates unchanged; only the leaf agent owns the snapshot). v1 constraint: one outstanding interrupted call per resume — multi-tool turns where the first call interrupts leave subsequent calls unexecuted. See [docs/hitl.md](docs/hitl.md).
- **Typed `humanApproval { }` gate (#2489)** — sugar over `interrupt(ApprovalRequest(...))` for the most common HITL pattern. `ApprovalBuilder` DSL (`title`, `body`, `timeout`, `defaultOnTimeout`) plus a sealed `HumanDecision { Approved, Rejected, Edited(payload), Responded(payload) }` resume value (not a boolean — `Edited`/`Responded` carry typed payloads). Two new audit events fire automatically via `Agent.observe { }`: `PipelineEvent.ApprovalRequested(title, hasBody, timeoutMs)` before the throw, and `PipelineEvent.ApprovalDecided(decision, hasPayload)` on resume when `resumeWith` is a `HumanDecision`. Field-only by design: bodies and payloads stay off the audit row (high-volume / PII discipline). Bridges (OTel / LangSmith / Langfuse) emit corresponding `agent.approval.requested` / `agent.approval.decided` spans / events / observations. Timeout is advisory — the caller honours it via `defaultOnTimeout` (defaults to `Rejected` — fail-closed for sensitive actions). See [docs/hitl.md](docs/hitl.md).
