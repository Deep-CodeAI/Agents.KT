...
- **Snapshot / resume** — `persistence { store = FileSnapshotStore(Path(...)); autoSnapshot = OnTurnComplete; restoreGuard = RestoreGuardPolicy.Strict }` checkpoints the message history + loop counters + memory at every turn boundary; `agent.resumeOrStart(sessionId, input)` continues a prior session or starts a fresh one. `restoreGuard` (default `Strict`) refuses to resume a snapshot whose `manifestHash` disagrees with the current agent's so a re-shaped agent (new tools, widened policy) cannot silently continue a session it never agreed to (#2418, #2419, builds on #2416). Off by default; plain `invokeSuspend(input)` is byte-for-byte unchanged. Composition snapshots are the next phase on #2386. See [docs/persistence.md](docs/persistence.md).
