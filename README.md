...
The 0.6 line turns those boundaries into audit-ready evidence: deterministic permission manifests, runtime `manifestHash` correlation, JSONL audit export, OTel / LangSmith / Langfuse bridge adapters, before-interceptor policy hooks, declarative tool policy metadata, a reasoning/thinking stream that's separate from the answer, vendor-neutral prompt caching with a prefix-stability guard, and a snapshot/resume foundation for durable agentic runs. Agents.KT is the runtime behind [agents-kt.dev](https://agents-kt.dev/).
...
// build.gradle.kts
dependencies {
    implementation("ai.deep-code:agents-kt:0.6.3")
}
```
...
- It does not sandbox arbitrary Kotlin lambdas in 0.6.x. `ToolPolicy` records intended filesystem/network/environment scope; OS/container enforcement remains a deployer responsibility until #1916.
...
- **Declarative tool policies** — `tool { policy { risk = ToolRisk.Medium; filesystem { read("/uploads/**") }; network { denyAll() } } }` records expected filesystem/network/environment scope for manifests and audit events. Declarative only in 0.6.x; sandbox enforcement is separate (#1915, #1916).
...
`main` is currently `0.6.3` — *"Prompt-caching foundation + Koog-bug regression net"* on top of the 0.6.x audit platform.

**New in the 0.6.3 batch.** **Prompt caching across providers**: `caching { enabled = true; cacheSystemPrompt = true; cacheToolDefs = true; cacheConversation = Rolling; ttl = 1.hours; cacheable("doc-id") { ... } }` declares cacheable prefix segments in vendor-agnostic terms; adapters translate (Anthropic explicit `cache_control` breakpoints with TTL ≤5min → ephemeral / >5min → `1h` and the per-request 4-breakpoint cap respected, OpenAI / DeepSeek automatic prefix caching with a stable agent-derived `prompt_cache_key` routing hint, Ollama / vLLM / SGLang engine-level KV-cache reuse). A **prefix-stability guard** hashes each cache-hinted segment per-agent across invocations and warns on drift, plus a first-sight pattern probe catches Unix-millis timestamps, ISO-8601 datetimes, and UUIDs in cacheable content — the silent killers that produce a full-price charge with no provider error. **Cache observability** lands on `TokenUsage` with `cacheWriteTokens` (Anthropic's premium-billed write side) and a derived `cacheHitRate`; cumulative usage across the agentic loop sums both sides. **Koog issue-set regression suite** (#2474) — eight regression tests pinning Agents.KT contracts where Koog broke: typed-arg recovery, unknown-tool-name recovery (now a tool-message + continue, was a fatal throw), `maxConsecutiveSameTool` name-only semantics, OpenRouter-style streaming chunk reconstruction, enum `@SerialName`-style verbatim emission, sealed `@Generable` parent-class type-discriminator dispatch (was returning null — a real correctness bug — now reaches the variant), stringified-JSON coercion for nested object/list/sealed fields, MCP `tools/call` JSON-serialized `@Generable` output (no more `SearchPayload(text=...)` debug leak). **`SessionHistory` accessors** (#2485) — `class SessionHistory(events: List<AgentEvent<*>>)` exposes `toolCalls()` / `toolResults()` / `assistantMessages()` / `completedOutput()` / `failed()` / `skillsStarted()` as thin, stable views over the event log.

**0.6.1 batch bundled into 0.6.3.** **Reasoning/thinking stream**: opt-in `model { reasoning(budgetTokens = …, effort = …) }` surfaces a model's reasoning on its own `AgentEvent.Reasoning` channel separate from the answer `Token` stream — Claude (extended thinking, forces T=1), DeepSeek (`reasoning_content`), and Ollama (`think:true`) emit reasoning text; OpenAI Chat Completions reports `reasoning_effort` + `reasoningTokens` only (no text on the wire). **`onBudgetExceeded`** raises a budget cap and continues mid-run via `BudgetDecision.Extend(newLimit)` instead of throwing — currently wired for the tool-call cap. **`onToolDenied`** + `PipelineEvent.ToolDenied` make blocked tool calls (via `onBeforeToolCall` `Decision.Deny`) first-class observable; previously dropped silently. **Typed parameter schemas** for built-in tools (`memory_write` / `memory_search` / `memory_read` / `forum_return` / swarm `absorb`) — no more reliance on providers' permissive empty-properties fallback. **Snapshot/resume foundation** (experimental): `Snapshotable<S>`, `SessionSnapshot`, `SnapshotStore` (`InMemorySnapshotStore` + `FileSnapshotStore` with atomic temp-write/rename), plus `executeAgentic` turn-boundary checkpoint + `resumeFrom` parameters — `internal` today; the public seam lands in #2749.

**0.6.0 audit platform.** **Permission manifest**: `:agents-kt-manifest` emits deterministic JSON/YAML capability graphs for agents and compositions, masks provider secrets, verifies high-risk widening in CI, attaches the manifest SHA-256 to runtime audit context. **Token usage telemetry**: `onTokenUsage { usage -> }` exposes provider-reported `TokenUsage(promptTokens, completionTokens, cachedInputTokens, provider, model)` once per successful LLM round-trip. **JSONL audit export**: `:agents-kt-observability` writes canonical append-only audit rows with `requestId` / `sessionId` / `manifestHash` correlation and PII-safe default field selection. **Observability bridge**: `:agents-kt-observability` exposes `ObservabilityBridge` and `.observe(bridge)`; `:agents-kt-otel` maps runtime events + before-interceptor decisions to OpenTelemetry spans, `:agents-kt-langsmith` maps the same events to LangSmith run trees, `:agents-kt-langfuse` maps them to Langfuse traces/generations/spans/events without adding any vendor to the core classpath. **DeepSeek provider**: `model { deepseek(...) }` joins Ollama, Anthropic, and OpenAI as the fourth built-in `ModelClient`. **Declarative tool policy**: `ToolPolicy` records tool risk plus filesystem/network/environment declarations for manifest/audit consumers; enforcement remains #1916. **Provider constrained decoding**: agentic skills returning `@Generable` types pass JSON Schema to supporting providers automatically (OpenAI `response_format.json_schema`, Ollama `format`, Anthropic structured-output tool), with parser retries retained as defense-in-depth. **Before-interceptors**: `onBeforeSkill` / `onBeforeTurn` / `onBeforeToolCall` return `Decision { Proceed | ProceedWith | Deny | Substitute }` for dynamic policy, prompt filtering, argument mutation, and synthetic results.

**0.5.0 platform underneath.** **Streaming runtime**: `agent.session(input).events: Flow<AgentEvent<OUT>>` surfaces typed `Token` / `ToolCall*` / bracket events as the agentic loop runs, with `requestId`, `sessionId`, and `manifestHash` on every event. Ollama, Anthropic, OpenAI, and DeepSeek stream at the wire (DeepSeek through the OpenAI-compatible SSE path). Every composition operator (`then` / `wrap` / `Branch` / `Loop` / `Parallel` / `Forum` / `Swarm`) surfaces sessions with `agentId`-tagged inner events. **MCP-as-skills unification**: `mcp.toolSkills()` + `mcp.promptSkills()` + `mcp.resourceSkills()` — every MCP capability shape exposes as a `Skill` consumable in `skills { +... }`. `McpServer` ships DSLs to register prompts and resources alongside agents-as-tools, inbound bearer auth, Host/Origin allowlists, per-principal tool policy, plus `McpStdioServer` and `McpRunner --stdio` over line-delimited stdio. `McpServerInfo` snapshots the full capability matrix. The 0.4 line (kotlin-reflect compileOnly, KSP `@Generable`, BouncyCastle hardening, `wrap` operator, original three providers) is included.
