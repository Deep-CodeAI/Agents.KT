...
- **Snapshot / resume** — `persistence { store = FileSnapshotStore(Path(...)); autoSnapshot = OnTurnComplete }` checkpoints the message history + loop counters + memory at every turn boundary; `agent.resumeOrStart(sessionId, input)` continues a prior session or starts a fresh one. Off by default; plain `invokeSuspend(input)` is byte-for-byte unchanged (#2418, builds on #2416). Manifest-hash restore guard and composition snapshots are the next phases on #2386. See [docs/persistence.md](docs/persistence.md).
