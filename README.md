...
- **Declarative policy DSL** — `agent { policy { requireHumanApprovalFor("send_email", "deploy"); redact("apiKey", "password") } }` (#2490). Sugar over existing surfaces: `requireHumanApprovalFor` triggers a `humanApproval { }` pause before the listed tools run (resume with a sealed `HumanDecision { Approved | Rejected | Edited(payload) | Responded(payload) }` — Approved/Edited execute the tool, Rejected/Responded short-circuit); `redact` is read by LangSmith / Langfuse bridges to scrub matching argument fields with `"[REDACTED]"` before audit writeout; the policy block is emitted into the permission manifest under `"policy"`, sorted for byte-determinism, and covered by the manifest SHA-256 hash so an auditor can verify the running system enforces the signed-off policy. Tied into #2754's restore guard — a snapshot taken under one policy can't silently replay against a different one. See [docs/policy.md](docs/policy.md).
