...
- **ObservabilityBridge adapters** — `.observe(OtelBridge(tracer))` maps runtime events to OTel spans (#1908), and `.observe(LangSmithBridge(apiKey, project))` maps the same events to LangSmith run trees with async batch dispatch (#1909), while keeping core vendor-free. See [docs/observability.md](docs/observability.md).
...
- [**Observability**](docs/observability.md) — JSONL audit exporter, `ObservabilityBridge`, OTel, and LangSmith adapters.
