...
- **Seven model providers** — `model { ollama(...) }` for local/cloud Ollama, `model { claude("claude-opus-4-7"); apiKey = ... }` for Anthropic's Messages API, `model { openai("gpt-4o"); apiKey = ... }` for OpenAI Chat Completions, `model { deepseek("deepseek-v4-flash"); apiKey = ... }` for DeepSeek's OpenAI-compatible API, `model { kimi("moonshot-v1-8k"); apiKey = ... }` for Kimi (Moonshot AI) — long-context (`-32k` / `-128k`) variants via the model string (#2697), `model { openrouter("anthropic/claude-3.5-sonnet"); apiKey = ...; openRouterHttpReferer = "https://your.app"; openRouterXTitle = "Your App" }` for OpenRouter — the OpenAI-compatible multi-provider aggregator that fronts hundreds of upstream models behind one API, with optional `HTTP-Referer` / `X-Title` attribution headers honored across the catalog (#2701), and `model { perplexity("sonar"); apiKey = ... }` for Perplexity's Sonar web-grounded API (OpenAI-compatible; `sonar` / `sonar-pro` / `sonar-reasoning-pro` / `sonar-deep-research`, #3675). All seven go through one `ModelClient` interface — `LlmMessage` / `LlmResponse` are provider-agnostic, tools/system/role mapping is per-adapter (#1644, #1656).
...
`main` is currently `0.7.24` — **Perplexity: seventh provider + web-grounded search with citations.** The new `PerplexityClient` (#3675) wires `api.perplexity.ai` Sonar models behind the same `model { perplexity("sonar") }` slot as the other connectors — Sonar / Sonar Pro / Sonar Reasoning Pro / Sonar Deep Research — and unlike Kimi/DeepSeek keeps constrained decoding **on** since Perplexity accepts OpenAI's `response_format` json_schema. The `perplexitySearch` tool (#3676) lets an agent on its own model fetch live, cited web facts; results carry `untrustedOutput = true` so they flow through the JSON envelope as data (not instructions), and the parsed source list reaches both the model context and the JSONL audit row. `perplexitySearchOptions { }` (#3677) maps to the documented request params — `search_mode`, recency filter, domain allow/deny, context size, reasoning effort, native typed `structuredOutput(MyType::class)`. Plus: a docs-accuracy pass that fixes false-negative roadmap signals (multimodal / streaming / session model marked unchecked while shipped), the `PerplexitySearch.kt` one-type-per-file split (#3199 burndown complete; CI guard now empty-allowlist), detekt baseline 415 → 410 (five real cleanups, no mechanical wraps), and dependency bumps — Kotlin 2.4.0, jline 4, detekt 1.23.8, KSP 2.3.9. Drop-in on the 0.7.x line; additive only.
