...
- **Snapshot / resume** — `persistence { store = FileSnapshotStore(Path(...)); autoSnapshot = OnTurnComplete; restoreGuard = RestoreGuardPolicy.Strict }` checkpoints the message history + loop counters + memory at every turn boundary; `agent.resumeOrStart(sessionId, input)` continues a prior session or starts a fresh one. `restoreGuard` (default `Strict`) refuses to resume a snapshot whose `manifestHash` disagrees with the current agent's so a re-shaped agent (new tools, widened policy) cannot silently continue a session it never agreed to (#2418, #2419, builds on #2416). **Composition snapshots** extend the same pattern to `Pipeline` / `Loop` / `Branch`: `pipeline.resumeOrStart(sessionId, input, CompositionSnapshotStore)` skips completed stages, `loop.resumeOrStart` skips completed iterations, `branch.resumeOrStart` reuses the source output and only re-runs the route (#2420; Forum + typed intermediates remain on #2386). Off by default; plain `invokeSuspend(input)` is byte-for-byte unchanged. See [docs/persistence.md](docs/persistence.md).
...
- [**Snapshot / Resume**](docs/persistence.md) — `persistence { store = ... }` + `Agent.resumeOrStart(sessionId, input)`; turn-boundary checkpointing, `InMemory` / `File` stores, what's persisted and what's not.
