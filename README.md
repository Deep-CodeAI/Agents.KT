...
- **Human-in-the-loop (HITL)** — a tool inside an agentic loop can call `interrupt(payload)` to pause; the caller catches `AgentInterruptException`, persists the snapshot (any `SnapshotStore`), asks a human, then resumes via `agent.invokeSuspendResuming(input, resumeFrom = snapshot, resumeWith = reply)` (#2488). `humanApproval { title = ...; body = ...; timeout = ...; defaultOnTimeout = ... }` is the typed sugar on top — sealed `HumanDecision { Approved, Rejected, Edited(payload), Responded(payload) }` resume type, field-only `PipelineEvent.ApprovalRequested` / `ApprovalDecided` audit events through `observe()`, default fail-closed timeout policy (#2489). Composes with the manifest-hash restore guard (#2754) — refuses to resume across an agent rebuild unless the caller opts in. See [docs/hitl.md](docs/hitl.md).
