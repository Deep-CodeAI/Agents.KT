...
- **Runtime attribution for trace correlation** — `AgentRuntimeContext.attribution: Map<String, String>` plus typed accessors (`userId` / `projectId` / `dialogId` via `AttributionKeys.*`) carry deployer-defined business identifiers alongside `requestId` / `sessionId` / `manifestHash`. Wrap any invocation in `withAgentRuntimeContext(currentOrNew().copy(attribution = mapOf(...)))` and every nested `AgentEvent` / `PipelineEvent` surfaces it via convenience getters — observability bridges (Langfuse `userId` filter, LangSmith run attribution, OTel resource attributes) drop their per-bridge side-channel `ConcurrentHashMap` + `onBeforeTurn` capture in favor of reading attribution directly off the runtime context (#2720). Non-breaking; defaults to empty.
