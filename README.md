...
- It does not sandbox arbitrary Kotlin lambdas in the 0.6.x line. `ToolPolicy` records intended filesystem/network/environment scope; OS/container enforcement remains a deployer responsibility until #1916.
...
- **Declarative tool policies** — `tool { policy { risk = ToolRisk.Medium; filesystem { read("/uploads/**") }; network { denyAll() } } }` records expected filesystem/network/environment scope for manifests and audit events. Declarative only in the 0.6.x line; sandbox enforcement is separate (#1915, #1916).
...
- **Document attachments (Anthropic native)** — `attachments = listOf(Content.Document(ref, DocMime.Pdf))` (#2470 slice c). Claude consumes PDF / plain text / markdown via its typed `document` content block — PDF rides as base64, text + markdown ride as decoded raw text on the `text` source (Anthropic only accepts base64 for PDFs). OpenAI Chat Completions and Ollama have no document-input format on `/chat`; their adapters drop documents silently with a one-shot JUL warning naming the workaround (extract text client-side, render to image, or switch to Claude). `Docx` / `Html` `DocMime` variants drop at the agent deref — converting them is a deployer-side toolchain concern. Live Claude tests: codename extraction from plain text + bullet-count from markdown. See [docs/multimodal.md](docs/multimodal.md#document-attachments--claude-pdf--text--markdown-2470-slice-c).
